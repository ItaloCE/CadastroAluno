...
class AlunoBanco:
    ...
    def criarConexao(self, host: str, username: str, db: str, password: str):
        try:
            conn = pymysql.connect(host=host, user=username, db=db, password=password, cursorclass=pymysql.cursors.DictCursor)
            return conn
...
if __name__ == '__main__':
    ...
    aluno1 = Aluno('Italo', 19, 'Java', 8.5)
    aluno2 = Aluno('João', 42, "Python", 9.0)
    a.insert(aluno1)
    a.insert(aluno2)
    print(a.select())
    aluno1.nome = 'Italo Silva'
    a.updade(aluno1)
    print(a.select())
